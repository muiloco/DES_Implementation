...
def permutacao(tipodapermutacao, chave):
    saida = ""
    for i in tipodapermutacao.value:
        saida += chave[i - 1]
    return saida
...
def substituicao(bloco):
    assert len(bloco) == 48, "O bloco nao e de 48 bits"
    saida = ""
    for i in range(8):
        si = bloco[i*6:(i+1)*6]
        linha = int(si[0]+si[5],2)
        coluna = int(si[1:5],2)
        saida = saida + "{0:b}".format(s[i][linha][coluna]).zfill(4)
    return saida